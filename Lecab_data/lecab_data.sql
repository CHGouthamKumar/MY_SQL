...
select * from cars join riders;

-- 4. using left join combine trips and riders
select * from trips left join riders using (id);

-- 5. use INNER join to combine trips and cars.
select * from trips join cars;
select * from trips inner join cars on trips.id =cars.id;

-- 6. Do union on riders and riders2 table.
select * from riders join riders2 union;

-- 7. What is the average cost of trip? round it in 2 decimals.
select avg(total_trips) from riders;
select avg(total_trips) from riders where total_trips decimal(2,3);
...
select * from cars where trips_completed > 100;
